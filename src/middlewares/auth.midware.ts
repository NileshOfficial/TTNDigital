...
dotenv.config();
export const retrieveAuthHeadersMidware = (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    try {
        if (!req.headers["authorization"])
            return next(
                new authExceptions.AuthHeaderAbsent(
        ...
        for (let idx = 0; idx < authHeader.length; idx++) {
            ...
            if (splitPieces[0].trim().toLowerCase() !== "bearer")
                return next(
                    new authExceptions.InvalidAuthHeaderFormat(
                        "auth tokens should be of bearer type",
                        401
                    )
                );
            else if (!splitPieces[1])
                return next(
                    new authExceptions.AuthTokenAbsent(
                        "auth token missing",
                        401
                    )
                );
...
export const verifyTokenMidware = async (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    try {
        await axios.get(
            uris.oAuthTokenInfoUri +
                `?access_token=${req["retrievedHeaders"]["access_token"]}`
        );
        return next();
    } catch (err) {
        return next(
...
export const validateIdTokenMidware = async (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    if (!req["retrievedHeaders"]["id_token"])
        return next(
            new authExceptions.AuthTokenAbsent("auth token missing", 401)
        );

    try {
        req["userProfile"] = verify(
            req["retrievedHeaders"]["id_token"],
...
const _verifyUserRole = async (userProfile: any) => {
    const userDetail = { email: userProfile.email } as User;
    try {
        const user = await findOrAddUser(userDetail);
